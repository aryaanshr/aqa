# In the __GetCell method of the Puzzle class, I added boundary checks to ensure the calculated Index is within the valid range of the grid. 
# If it's not, a message is displayed to the user indicating the error, and None is returned.
# Modified the AttemptPuzzle method to continuously prompt the user for row and column inputs until valid coordinates are provided.
# For this, I added a while loop around the row and column input sections to keep asking the user until a valid cell (CurrentCell) is returned by the __GetCell method.
# and then just created another method to keep getting the input from the user.
import random
import os
...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            # Modified this section to keep prompting the user until a valid row and column are provided
            CurrentCell = None
            while CurrentCell is None:
                Row = self.__GetUserInput("Enter row number: ")
                Column = self.__GetUserInput("Enter column number: ")
                CurrentCell = self.__GetCell(Row, Column)
                
            Symbol = self.__GetSymbolFromUser()
            self.__SymbolsLeft -= 1
            if CurrentCell and CurrentCell.CheckSymbolAllowed(Symbol):
    ...
    def __GetCell(self, Row, Column):
        ...
        if 0 <= Index < len(self.__Grid):
            return self.__Grid[Index]
        else:
            print("Error: Invalid row or column input. Please enter a valid position.")
            return None
