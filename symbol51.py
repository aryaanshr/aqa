...
def Main():
    ...
    while Again == "y":
        ...
        if len(Filename) > 0:
            MyPuzzle = ModifiedPuzzle(Filename + ".txt")
        else:
            MyPuzzle = ModifiedPuzzle(8, int(8 * 8 * 0.6))
...
class ModifiedPuzzle(Puzzle):
    def __init__(self, *args):
        super().__init__(*args)
        self.__turns = 0
        self.__bomb_used = False
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            self.DisplayPuzzle()
            print("Current score: " + str(self.__Score))
            print(f"Turns taken: {self.__turns}")

            if self.__turns >= 15 and not self.__bomb_used:
                use_bomb = input("Do you want to use the free bomb (y/n)? ").lower()
                if use_bomb == 'y':
                    self.__use_free_bomb()
                    self.__bomb_used = True
                    self.__turns += 1
                    continue
    ...
    def __use_free_bomb(self):
        print("Select the center of the 5x5 grid to clear.")
        Row = -1
        Valid = False
        while not Valid:
            try:
        ...
        for r_offset in range(-2, 3):
            for c_offset in range(-2, 3):
                try:
                    cell = self.__GetCell(Row + r_offset, Column + c_offset)
                    if not isinstance(cell, BlockedCell):
