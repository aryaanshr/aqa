# Modified the __GetSymbolFromUser method inside the Puzzle class to accept 'B' as input, which represents the bomb. 
# Then I modified the AttemptPuzzle method to handle the bomb input. If the user chooses 'B' and selects a blocked cell , that cell is replaced with a normal cell and 5 points are then subtracted from the score.
...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if Symbol == 'B':
                self.__SymbolsLeft -= 1
                if isinstance(CurrentCell, BlockedCell):
                    self.__Grid[(self.__GridSize - Row) * self.__GridSize + Column - 1] = Cell()
                    self.__Score -= 5
                continue
