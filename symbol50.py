# since this is a little more difficult, I wll have to plan things - 
# 1 - Before a change is made to the puzzle (e.g., a symbol is added), the current state of the puzzle should be saved, so that means we also need to store the number of symbols left (maybe not) and also the scoree
# 2 - we have to load this saved state
# 3 - put it into the game loop

# so i saved it as a dictionary in the variable self.__previousState
...
class Puzzle():
    ...
    def save_state(self):
        state = {
            'grid': [cell._Symbol for cell in self.__Grid],
            'score': self.__Score,
            'symbols_left': self.__SymbolsLeft
        }
    ...
    def undo_last_move(self):
        ...
        self.__Score = last_state['score'] - 10
        self.__SymbolsLeft = last_state['symbols_left']
        for i, symbol in enumerate(last_state['grid']):
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            undo = input("Type 'undo' to revert the last move (costs 10 points) or press Enter to continue: ").lower()
            if undo == "undo":
                self.undo_last_move()
                continue
            self.save_state()
