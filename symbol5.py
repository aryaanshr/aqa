...
def Main():
  ...
  while Again == "y":
    ...
    if len(Filename) > 0:
      ...
      MyPuzzle = Puzzle(8, int(8 * 8 * 0.1))
    Score = MyPuzzle.AttemptPuzzle()
    print("Puzzle finished. Your score was: " + str(Score))
    Again = input("Do another puzzle? ")
...
class Puzzle():
  ...
  def __init__(self, *args):
    if len(args) == 1:
      ...
      self.__AllowedPatterns = []
      self.__AllowedSymbols = []
      QPattern = Pattern("Q", "QQ**Q**QQ")  #asterix go round in a circle
  ...
  def AttemptPuzzle(self):
    ...
    ##START##
    Pause = False
    while not Finished and Pause == False:
      self.DisplayPuzzle()
      print("Moves Left: " + str(self.__SymbolsLeft))
      print("Current score: " + str(self.__Score))
      next = input("what do u wanna do?: ").upper()

      if next == "S" or next == "SAVE":

        with open('save.txt', 'w') as f:
          f.write(str(len(self.__AllowedSymbols)))
          for i in range(len(self.__AllowedSymbols)):
            f.write("\n" + str(self.__AllowedSymbols[i]))

          f.write(f"\n{len(self.__AllowedPatterns)}")
          for EachPattern in self.__AllowedPatterns:
            f.write(
                f"\n{EachPattern.GetSymbol()},{EachPattern.GetPatternSequence()}"
            )

          f.write(f"\n{self.__GridSize}")
          for EachCell in self.__Grid:
            if EachCell.GetSymbol() == "-":
              if len(EachCell.NotAllowedSymbols()) == 0:
                f.write(f"\n,")
              else:
                f.write("\n")
                for EachSymbol in EachCell.NotAllowedSymbols():
                  f.write(f",{EachSymbol}")
            else:
              if len(EachCell.NotAllowedSymbols()) == 0:
                f.write(f"\n{EachCell.GetSymbol()},")
              else:
      ...
      for StartColumn in range(Column - 2, Column + 1):
        try:
          ...
          PatternString += self.__GetCell(StartRow,
                                          StartColumn + 1).GetSymbol()
          PatternString += self.__GetCell(StartRow,
                                          StartColumn + 2).GetSymbol()
          ...
          for P in self.__AllowedPatterns:
            ...
            if P.MatchesPattern(PatternString, CurrentSymbol):
              self.__GetCell(StartRow,
                             StartColumn).AddToNotAllowedSymbols(CurrentSymbol)
    ...
    for Count in range(0, len(self.__PatternSequence)):
      try:
        if self.__PatternSequence[
            Count] == self.__Symbol and PatternString[Count] != self.__Symbol:
          return False
...
class BlockedCell(Cell):  #inheritence
