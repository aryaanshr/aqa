...
class Puzzle():
    def __init__(self, *args):
        if len(args) == 1:
            ...
            self.__PatternCounters = {pattern_symbol: 0 for pattern_symbol in self.__AllowedSymbols}
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            if AmountToAddToScore > 0:
                ...
                self.__PatternCounters[CurrentSymbol] += 1
                if self.__PatternCounters[CurrentSymbol] == 2:
                    self.__SymbolsLeft += 1
                    self.__PatternCounters[CurrentSymbol] = 0
                AmountToAddToScore = self.CheckforMatchWithPattern(Row, Column)
                if AmountToAddToScore > 0:
                    self.__Score += AmountToAddToScore
