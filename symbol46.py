# I added an input prompt at the start of the loop in AttemptPuzzle method which asks the user if they want to finish the game. 
# If the user enters 'F', it adds the score for the remaining symbols to the total score and finishes the game. 
# When the user chooses to finish the game, the score for the remaining symbols is calculated as self.__SymbolsLeft * 0.5 and added to the current score.
# And then the score is obviously printed out!
...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            action = input("Enter 'F' to finish or any key to continue: ").lower()
            if action == 'f':
                self.__Score += self.__SymbolsLeft * 0.5
                Finished = True
                continue
            Row = -1
            Valid = False
