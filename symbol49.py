#  I added a check to determine if the selected cell is already occupied using the IsOccupied metho
# If the cell is occupied, the program displays an error message
# IsOccupied is a new method that I put into Cell class to find out whether a 
...
class Puzzle():
    ...
    def AttemptPuzzle(self):
        ...
        while not Finished:
            ...
            # Asking user for row and column
            Row = self.__GetValidInput("Enter row number: ")
            Column = self.__GetValidInput("Enter column number: ")
            ...
            if not CurrentCell.IsEmpty():
                print("Error: Cell already has a symbol. Please choose another cell.")
                continue
                
            if CurrentCell.CheckSymbolAllowed(Symbol):
                CurrentCell.ChangeSymbolInCell(Symbol)
                self.__SymbolsLeft -= 1  # Decrement the counter only if the symbol is placed successfully
                AmountToAddToScore = self.CheckforMatchWithPattern(Row, Column)
                if AmountToAddToScore > 0:
                    self.__Score += AmountToAddToScore
            else:
                print("Error: Symbol not allowed in this cell. Please choose another cell.")
        ...
        print()
        return self.__Score
    def __GetValidInput(self, prompt_msg):
        value = -1
        Valid = False
        while not Valid:
            try:
                value = int(input(prompt_msg))
                if 1 <= value <= self.__GridSize:
                    Valid = True
                else:
                    print(f"Invalid input. Please enter a value between 1 and {self.__GridSize}.")
            except:
